...
**Size:** S · **Labels:** etl, docs · **Depends on:** CS-109 · **Owner:** Terrence · **Status:** Partly done
...
- Manifests persisted to a table so history is queryable, not just the latest run.
  **Not started.**
- The generated block in `docs/provenance.md` regenerated by the nightly job from
  `PullMetadata.provenance_row()`, so the page cannot drift from the data.
  **Not started.**
- Exposed through an endpoint for the frontend to consume in CS-406.
  **Not started.**
- Values are written by the pipeline itself, never maintained by hand.
