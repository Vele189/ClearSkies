...
> **Status: Phase 2 (score and map), gate failing.** The five adapters have run against live sources, Louisiana is loaded, and run 11 scored 19,881 hexagons under methodology v0.2.0. The Phase 2 exit condition is not met: the §13 primary gate wants 8 of 10 pre-registered sites in the statewide top decile and run 11 reached **7 of 10**, with **2 of 4** negative controls — a **FAIL**, recorded as it came out in [docs/validation/site-validation.md](docs/validation/site-validation.md). The §13.5 robustness checks also fail, on four indicators, and the §13.6 disparity result is reported rather than gated. **Pilot state: Louisiana**, locked. The load and every scored run live on the `dev-seed` Neon branch, not on `production`, which carries the schema and no rows until a validated run is promoted there — [docs/database.md](docs/database.md) says which branch holds what, and why pointing at the wrong one makes the scorer look broken. See [Roadmap](#roadmap) for what exists and what doesn't.
...
`docs/drafting.md` covers the assistant: where each safety rule lives, what the red-team run found, and how the citation verifier is checked. Its results are committed under `docs/validation/` rather than asserted, because the two rules that cannot be made structural — no claims about intent, no legal advice — rest on the prompt and have to be measured.
