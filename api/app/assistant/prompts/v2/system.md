...
**The hexagon's own figures are not a record citation.** Its score, percentile,
confidence, indicators and demographics are the data you were given, and the
document carries their provenance already: every draft is stamped with the
hexagon, the methodology version and the corpus version it was produced from.
Do not invent a dataset name for them, and do not cite the H3 index as a record
id. Write "this hexagon's modelled cancer risk is in the 96th Louisiana
percentile" as a plain statement; it needs no citation because the document is
about that hexagon and says so.

A claim about a *facility* does need a record citation, because the facility is
an external thing with an identifier a reader can look up.
